...
// Node.js __dirname workaround in ES Modules
...
const inputIndex =
  args.indexOf("--read") !== -1 ? args.indexOf("--read") : args.indexOf("--r");
const outputIndex =
  args.indexOf("--make") !== -1 ? args.indexOf("--make") : args.indexOf("--m");
const watchMode =
  args.includes("--watch") || args.includes("--w") || args.includes("--wacth");
...
if (
  ...
  console.error(
    "❌ Usage: node generate.js [--read|--r] input.html [--make|--m] output.html [--watch|--w]"
  );
  process.exit(1);
}
...
async function processFile() {
  console.log(
    `🔄 Processing ${inputFile} at ${new Date().toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata" })}`
  );
  warnings = [];
  ...
  const allLink = Array.from(document.querySelectorAll("link"))
  .map(tag => tag.outerHTML)
  .join('\n');

  const allMeta= Array.from(document.querySelectorAll("meta")).map(tag => tag.outerHTML)
  .join('\n');
  
  let createTitleElement = "";
  const titleTag = document.querySelector("title");
  
  if (!titleTag) {
    createTitleElement = "<title>NextHtml</title>";
  } else {
    createTitleElement = titleTag.outerHTML;
  }
  ...
  // Generate script for browser console
  let script = "";
  if (warnings.length) {
    const warningScript = warnings
      .map((msg) => `console.warn(${JSON.stringify(msg)});`)
      .join("\n");
    script = `<script>\n${warningScript}\n</script>`;
  }
...
`;
  ...
  let formattedOutput;
  try {
    formattedOutput = await prettier.format(output, { parser: "html" });
  } catch (err) {
  ...
  try {
    ...
    console.log(
      `✅ ${outputFile} updated at ${new Date().toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata" })}`
    );
  } catch (err) {
...
if (watchMode) {
  ...
  watcher
    .on("change", (path) => {
      console.log(
        `🔄 Detected change in ${path} at ${new Date().toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata" })}`
      );
    ...
    .on("ready", () => {
      console.log("✅ Watcher initialized, monitoring for changes...");
      processFile();
    });
} else {
  processFile();
